...
export default function UploadPage() {
  ...
  const [indexWarningFiles, setIndexWarningFiles] = useState([]);
  ...
  const readFile = (file) =>
    new Promise((resolve) => {
      ...
      console.log(
        `[UPLOAD] readFile: name="${file.name}" type="${file.type}" size=${file.size} bytes`
      );
      ...
      readerB64.onload = () => {
        ...
        console.log(`[UPLOAD] readFile: isTextLike=${isTextLike} for "${file.name}" — only text-like files get textContent extracted here. PDFs/images get NO text extraction (they only get a base64 preview for display).`);
        ...
        if (isTextLike) {
          ...
          readerText.onload = () => {
            ...
            console.log(
              `[UPLOAD] readFile: textContent extracted for "${file.name}" — length=${(result.textContent || "").length}`
            );
            resolve(result);
          };
          readerText.onerror = (err) => {
            console.error(`[UPLOAD] readFile: Text read FAILED for "${file.name}":`, err);
            resolve(result);
          };
          ...
          if (file.type === "application/pdf" || file.name.toLowerCase().endsWith(".pdf")) {
            console.log(
              `[UPLOAD] PDF "${file.name}" was read as a data-URL preview only. Server-side pdf-parse extraction in /api/memories will pull textContent from the preview before indexing.`
            );
          }
          console.log(`[UPLOAD] readFile: no client-side text extraction for "${file.name}" — textContent stays "" (length=0; server may extract for PDFs)`);
          resolve(result);
        }
      };
      readerB64.onerror = (err) => {
        console.error(`[UPLOAD] readFile: FileReader FAILED for "${file.name}":`, err);
        resolve(result);
      };
  ...
  const handleUpload = async () => {
    ...
    setSaving(true);
    try {
      const saved = await Promise.all(selectedFiles.map(({ file, preview, textContent }) =>
        addMemory({
      ...
      // Surface indexing failures instead of letting them pass as silent
      // success — a memory whose vector write failed is saved but won't be
      // searchable in chat. "no_text_content" (e.g. images) isn't a failure.
      const failedToIndex = saved
        .filter((m) => m.indexing && !m.indexing.indexed && m.indexing.reason !== "no_text_content")
        .map((m) => m.title);
      setIndexWarningFiles(failedToIndex);
  ...
  const goToNextFile = () => {
    ...
    setQaMessages([{ id: crypto.randomUUID(), role: "ai", text: `Ask me anything about "${savedMemories[nextIndex].title}".` }]);
    setQaInput("");
  };
  ...
  const sendQaQuestion = () => {
    ...
    const aiMsg = { id: crypto.randomUUID(), role: "ai", text: answerAboutFile(question, currentReaderFile) };
    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <div className="max-w-3xl mx-auto py-10">
        <div className="bg-white rounded-3xl shadow-xl p-10">
          ...
          {indexWarningFiles.length > 0 && (
            <div className="mb-6 bg-amber-50 border border-amber-200 text-amber-700 rounded-xl p-4 text-sm">
              Saved, but not searchable yet — the vector index is unavailable for:{" "}
              {indexWarningFiles.join(", ")}. You can still find {indexWarningFiles.length === 1 ? "it" : "them"} in Timeline; ask an admin to check the database connection to enable chat search.
            </div>
          )}
