...
export default function TimelinePage() {
  ...
  const confirmMoveDate = async () => {
    ...
    await updateMemories(getSelectedIds(), { date: moveDate, year: newYear });
    await refresh(); setShowModal(false); cancelSelection(); setMoveDate("");
  };

  const confirmMoveCategory = async () => {
    if (!selectedCategory) { alert("Please select a category."); return; }
    await updateMemories(getSelectedIds(), { category: selectedCategory });
    await refresh(); setShowModal(false); cancelSelection(); setSelectedCategory("");
  };

  const confirmRename = async () => {
    if (!renameValue.trim()) { alert("Please enter a name."); return; }
    await updateMemories(getSelectedIds(), { title: renameValue.trim() });
    await refresh(); setShowModal(false); cancelSelection(); setRenameValue("");
  };

  const confirmDelete = async () => {
    await deleteMemories(getSelectedIds());
    await refresh(); setShowModal(false); cancelSelection();
  };
